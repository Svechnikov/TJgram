...
import io.svechnikov.tjgram.features.timeline.usecases.RefreshTimeline
...
class TimelineViewModel @Inject constructor(
    ...
    val posts: LiveData<PagedList<PostView>> = switchMap(sortingMutable) {
        ...
        db.posts().posts(it).map {entity ->
            ...
            boundaryCallback = TimelineBoundaryCallback(queryTimeline,
                PAGE_SIZE, it, executors).apply {
                helper.addListener {
                    if (it.hasRunning()) {
    ...
    fun refresh() {
        sortingMutable.value?.let {
            ...
            refreshTimeline(RefreshTimeline.Params(it, PAGE_SIZE)) {result ->
    ...
    private fun onError(throwable: Throwable) {
        ...
        eventMutable.value = TimelineEvent.ShowError(getErrorMessage(throwable))
    }
}
