 ...
 */
package io.svechnikov.tjgram.base.utils;
 ...
 * <p>
 * It tracks a {@link Status} and an {@code event} for each {@link RequestType}.
 * <p>
 ...
 *     // merge multiple state per request type into one, or dispatch separately depending on
 *     // your application logic.
 *     if (status.hasRunning()) {
 *         combined.postValue(PagingRequestHelper.Status.RUNNING);
 *     } else if (status.hasError()) {
 *         // can also obtain the event via {@link StatusReport#getErrorFor(RequestType)}
...
public class PagingRequestHelper {
    ...
    public interface Request {
        ...
        class Callback {
             ...
             * {@link #retryAllFailed()}.
             *
             * @param throwable The event that occured while carrying out the request.
             */
            @SuppressWarnings("unused")
            public final void recordFailure(@NonNull Throwable throwable) {
                ...
                if (throwable == null) {
                    throw new IllegalArgumentException("You must provide a throwable describing"
                            + " the event to record the failure");
                }
                if (mCalled.compareAndSet(false, true)) {
    ...
    public static final class StatusReport {
        ...
        /**
         * Convenience method to check if there are any requests that resulted in an event.
         *
         * @return True if there are any requests that finished with event, false otherwise.
         */
        ...
        /**
         * Returns the event for the given request type.
         *
         * @param type The request type for which the event should be returned.
